...
class Database:
    ...
    def _connect_database(self, connection):
        ...
        if connection.startswith("mysql+pymysql"):
            ...
            self.engine = create_engine(self.db_url, connect_args={"sslmode": "disable"})
        else:
            db_path = os.path.join(__project__.get_path(), 'viper.db')
            self.db_url = 'sqlite:///{0}'.format(db_path)
...
def _migrate_db_to_alembic_management(alembic_cfg, engine, rev, verbose=False):
    """ migrate a non alembic database to a specified revision

    :param alembic_cfg: configured AlembicConfig object
    :type alembic_cfg: object
    :param engine: connected SQL Alchemy engine
    :type engine: object
...
def upgrade_database(database_url, sqlite=True, create_backup=True, verbose=False):
    if check_database(database_url):
        print_info("Already up2date!")
        return

    # First: create a backup - only if not already up2date
    if create_backup:
        backup_database(database_url, sqlite, verbose)
    ...
    if verbose:
        print_item("Connecting to Viper Databases: {}".format(database_url))
    engine = create_engine(database_url)
